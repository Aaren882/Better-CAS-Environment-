...
class BCE_Task_Receiver
{
	...
	class controlsBackground
	{
		class Background: RscPicture
		{
			...
			y = QUOTE(RECEIVERY);
			w = QUOTE(RECEIVERW);
			h = QUOTE(RECEIVERH - (1 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25)));
	...
	class controls
	{
		class Title: RscIGUIText
		{
			...
			x = QUOTE(RECEIVERX);
			y = QUOTE(RECEIVERY - ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25));
			w = QUOTE(RECEIVERW);
			h = QUOTE((((safezoneW / safezoneH) min 1.2) / 1.2) / 25);
		...
		class JTAC_Unit: RscIGUIText
		{
			...
			text = "From";
			style = 1;
			x = QUOTE(RECEIVERX + (0.25 * RECEIVERW));
			y = QUOTE(RECEIVERY - ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25));
			w = QUOTE(0.75 * RECEIVERW);
			h = QUOTE((((safezoneW / safezoneH) min 1.2) / 1.2) / 25);
			sizeEx = "0.8 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25)";
		};
