//- message Line
	class ATAK_Message_Line: RscStructuredText
	{
		...
		size = QUOTE(0.65 * (((60)) / 2048 * CustomPhoneH));
		class Attributes
		{
			color = "#dee0de";
		};
	...
	//- CFF Standard
		class ATAK_CFF_STD: ATAK_Group_Manage_Custom
		{
			...
			class controls: controls
			{
				...
				{
					h = QUOTE((3 * 0.7) * (((60)) / 2048 * CustomPhoneH));
				};
		...
		class controls
		{
			...
			//- Clear Button
				class Clear_Adjust: BCE_RscButtonMenu
				{
					...
					{
						left = QUOTE((0.35 * ATAK_POS_W / 2) - (0.75 * TextSize / 2));
						top = QUOTE(((ADJUSTMENT_MENU * 0.7 / 2) * ATAK_POS_H) - (0.75 * TextSize / 2));
						w = QUOTE(0.75 * TextSize);
						h = QUOTE((ADJUSTMENT_MENU * 0.7) * ATAK_POS_H / 4);
					};
					class TextPos: TextPos
					{
						top = QUOTE(((ADJUSTMENT_MENU * 0.7 * 0.5) * ATAK_POS_H) - (0.75 * TextSize * 0.5));
					};
					class Attributes: Attributes
					{
						align="center";
			...
			//- Adjustment Controls (MACROS)
				...
				#define ADJUST_BNT_POS(XPOS,YPOS,WPOS,HPOS) \
					x = QUOTE(PhoneBFTContainerW(XPOS) + (((ADJUST_INTERVAL/2) - ADJUST_INTERVAL) * pixelW)); \
					y = QUOTE(YPOS * ATAK_POS_H + (ADJUST_INTERVAL / 2 * pixelH)); \
					w = QUOTE(PhoneBFTContainerW(WPOS) - (ADJUST_INTERVAL * pixelW)); \
					h = QUOTE(HPOS * ATAK_POS_H - (ADJUST_INTERVAL * pixelH))
			...
			//- Middle (Indicators, Adjust...)
				class Indicator: RscPictureKeepAspect
				{
					...
					x = QUOTE(0.35 * ATAK_POS_W);
					y = QUOTE(0);
					w = QUOTE(((2.8 - 0.35) * ATAK_POS_W) - ADJUST_CTRL_W(0.4));
					h = QUOTE(1.5 * 0.7 * ATAK_POS_H);
				...
				class Indication: RscStructuredText
				{
					idc = 5002;
					x = QUOTE(0.35 * ATAK_POS_W);
					y = QUOTE(1.5 * 0.7 * ATAK_POS_H);
					w = QUOTE(((2.8 - 0.35) * ATAK_POS_W) - ADJUST_CTRL_W(0.525));
					h = QUOTE(0.5 * 0.7 * ATAK_POS_H);
			...
			//- Adjust Buttons -//
				//- Background
					...
					{
						x = QUOTE(((2.5 - BORDER) - 2 * ADJUST_BNT_W) * ATAK_POS_W + (((ADJUST_INTERVAL/2) - ADJUST_INTERVAL) * pixelW));
						y = QUOTE(0);
						w = QUOTE(ADJUST_CTRL_W(1));
						h = QUOTE(3 * 0.7 * ATAK_POS_H);
					};
...
//- IMPACT (ADJUST)
	class CFF_ADJUST_IMPACT_Group: CFF_ADJUST_POLAR_Group
	{
		...
		class controls: controls
		{
			...
			class Adjust_Bnt: Adjust_Bnt
			{
				x = QUOTE((2.8 * ATAK_POS_W) - (((2.8 - 0.35) * ATAK_POS_W) - ADJUST_CTRL_W(0.525)));
			...
			//- EditBox
				...
				{
					idc = 150;
					x = QUOTE((0.35 + 0.7) * ATAK_POS_W);
					y = QUOTE((4 * pixelH));
					w = QUOTE(((2.8 - 0.35 - 0.7) * ATAK_POS_W) - (4 * pixelW));
					h = QUOTE(0.7 * ATAK_POS_H - (2 * 4 * pixelH));
				...
				{
					idc = 160;
					y = QUOTE(0.7 * ATAK_POS_H + (4 * pixelH));
				};
			delete Indicator;
			delete AdjustBg;
			delete Adjust_Meter;
