...
//- Base ICON CONTROL
  class BCE_ATAK_Tool_ICON: BCE_RscButtonMenu
  {
    ...
    x = 0;
    y = 0;
    w = QUOTE(PhoneBFTContainerW(1));
    h = QUOTE((phoneSizeW * 3/5)/3);
    ...
    class ShortcutPos
    {
      left = QUOTE(0.75 * (((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) / TextTimesH));
      top = QUOTE((phoneSizeW * 3/5)/3*0.18);
      w = QUOTE((phoneSizeW * 2/5)/5*1.1);
      h = QUOTE((phoneSizeW * 3/5)/5);
    };

    class TextPos
    {
      left = "0.25 * (((safezoneW / safezoneH) min 1.2) / 40)";
      top = QUOTE(((phoneSizeW * 3/5)/3*0.18) + ((phoneSizeW * 3/5)/5));
      right = 0;
      bottom = 0;
    };
